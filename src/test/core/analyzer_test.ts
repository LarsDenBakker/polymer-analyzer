...
~~`);
    ...
    test(
        'an inline feature can narrow down its containing document',
        async () => {
          const fileName = 'static/script-tags/inline/test-element.html';
          const result = await analyzer.analyze([fileName]);
          const document = await result.getDocument(fileName) as Document;
          const inlineDocuments = Array
                                      .from(document.getFeatures(
                                          {kind: 'document', imported: false}))
                                      .filter((d) => d.isInline);
          ...
          const narrowedDocument =
              result.getDocumentContaining(subBehavior.sourceRange, document);
